...
describe('lib-resources-module.schematic', () => {
  ...
  it('should generate a resources module', async () => {
    ...
    expect(readRequiredFile(tree, RESOURCES_MODULE_FILE)).toContain(
      'resources: any = {"EN-US":{"foobar":"Hello, world!"}};'
    );
  });
  ...
  it('should overwrite the local resources module if it exists', async () => {
    ...
    expect(readRequiredFile(tree, RESOURCES_MODULE_FILE)).not.toContain(
      'ORIGINAL CONTENT'
    );
  });
  ...
  it('should add `@skyux/i18n` as a peer dependency', async () => {
    ...
    const packageJsonContents = JSON.parse(
      readRequiredFile(tree, PACKAGE_JSON_FILE)
    );
